<template>
    <div class="modal">
        ...
        <form id="form">
            ...
            <div>
                <input type="name" id="task-name" name="task-name" placeholder="Nome da tarefa" maxlength="35" required
                    v-model="stitle">
            </div>

            <div>
                <input type="name" id="task-description" name="task-description" maxlength="50" placeholder="Descrição"
                    required v-model="sdescription">
            </div>

            <div class="buttons">
                <router-link :to="'/dashboard'"><button class="white-button">Cancelar</button></router-link>               
                <div class="button" @click="updateSubTask()">
                    <input type="button" class="black-button" value="Atualizar subtarefa">
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        async updateSubTask() {
            ...
            const result = await axios.put(`http://localhost:8000/api/subtask/` + this.$route.params.id + `/update`,
                {
                    stitle: this.stitle,
